...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if (preg_match ('/^[A-Z \'.-]{2,20}$/i', $_POST['cc_first_name'])) {
                ...
                $cc_first_name= mysql_real_escape_string($cc_first_name);
                $cc_first_name = str_replace(array("'",'>','<','&','|',';','\\'), '',$cc_first_name);
	} else {
	...
	if (preg_match ('/^[A-Z \'.-]{2,40}$/i', $_POST['cc_last_name'])) {
                ...
                $cc_last_name= mysql_real_escape_string($cc_last_name);
                $cc_last_name = str_replace(array("'",'>','<','&','|',';','\\'), '',$cc_last_name);
	} else {
	...
	// Strip out spaces or hyphens:
        ...
        $cc_number= mysql_real_escape_string($cc_first_name);
	...
	if (preg_match ('/^[0-9]{3,4}$/', $_POST['cc_cvv'])) {
                $cc_cvv = addslashes($_POST['cc_cvv']);
                $cc_cvv= mysql_real_escape_string($cc_cvv);
                $cc_cvv = str_replace(array("'",'>','<','&','|',';','\\'), '',$cc_cvv);
	} else {
	...
	if (preg_match ('/^[A-Z0-9 \',.#-]{2,160}$/i', $_POST['cc_address'])) {
                $cc_address = addslashes($_POST['cc_address']);
                $cc_address= mysql_real_escape_string($cc_address);
                $cc_address = str_replace(array("'",'>','<','&','|',';','\\'), '',$cc_address);
	} else {
	...
	if (preg_match ('/^[A-Z \'.-]{2,60}$/i', $_POST['cc_city'])) {
                ...
                $cc_city= mysql_real_escape_string($cc_city);
                $cc_city = str_replace(array("'",'>','<','&','|',';','\\'), '',$cc_city);
	} else {
	...
	if (preg_match ('/^(\d{5}$)|(^\d{5}-\d{4})$/', $_POST['cc_zip'])) {
                $cc_zip = addslashes($_POST['cc_zip']);
                $cc_zip= mysql_real_escape_string($cc_zip);
                $cc_zip = str_replace(array("'",'>','<','&','|',';','\\'), '',$cc_zip);
	} else {
...
$page_title = 'We Are Nuts - Checkout - Your Billing Information';
